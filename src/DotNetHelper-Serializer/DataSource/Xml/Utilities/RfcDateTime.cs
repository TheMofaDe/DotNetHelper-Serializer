...
namespace DotNetHelper_Serializer.DataSource.Xml.Utilities
{
    ...
    internal static class RfcDateTime
    {
        public static string ToString(DateTime dateTime)
        {
            ...
            {
                var utcOffset = TimeZoneInfo.Local.GetUtcOffset(dateTime);  // TimeZone.CurrentTimeZone.GetUtcOffset(dateTime);  //TimeZoneInfo.Local.GetUtcOffset(dateTime);//.GetUtcOffset(dateTime);

                builder.Append(utcOffset.Ticks > 0 ? '+' : '-');
