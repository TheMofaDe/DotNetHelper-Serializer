...
Task("Heat")
  .IsDependentOn("Build")
   .Does<BuildParameters>((parameters) =>{
    ...
    DirectoryPath harvestDirectory = Directory($"{parameters.ProjectDir}bin/{parameters.Configuration}/{parameters.FullFxVersion}/");
   ...
   .Does<BuildParameters>((parameters) =>{
        ...
        var settings = new CandleSettings {
			ArgumentCustomization = args => args
                .Append("-dHeatSourceFilesDir=" + $"{parameters.ProjectDir}bin/{parameters.Configuration}/{parameters.FullFxVersion}/")
			  ,
            Verbose = true,
            NoLogo = true,
            OutputDirectory = parameters.Paths.Directories.ArtifactsBinFullFx,
		    Defines = new Dictionary<string, string>(){ 
						 {"SourceDir", parameters.Paths.Directories.ArtifactsBinFullFx.ToString() }
				  }            
        };
  ...
  .IsDependentOn("Candle")
     .Does<BuildParameters>((parameters) =>{
      LightSettings settings = new LightSettings {
        ...
        OutputFile = parameters.Paths.Directories.ArtifactsBinFullFx.ToString() + "/" +  parameters.ProjectName + "_Installer.msi",
        NoLogo = true,
     
        };
    WiXLight(parameters.Paths.Directories.ArtifactsBinFullFx.ToString() + "/*.wixobj", settings);
